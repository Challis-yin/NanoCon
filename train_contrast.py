...
from pytorch_lighting_model.lighting_old import model_encoder
...
if __name__ == '__main__':
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename="tha-mnist-{epoch:02d}-{avg_val_ACC:.5f}-{avg_val_AUPRC:.5f}-{avg_val_AUROC:.5f}-{avg_val_Precision:.5f}-{avg_val_Recall:.5f}-{avg_val_F1Score:.5f}",
        save_top_k=1,
        mode="max",
        save_last=False
    )
    ...
    loader = DataLoader(mnist_train, 2048, True, collate_fn=collate, num_workers=12)
    loader_valid = DataLoader(mnist_val, 2048, False, collate_fn=collate)
